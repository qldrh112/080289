...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, ouptut_size, dropout_p=0.1, max_length=MAX_LENGTH):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        # 가중치와 인코더의 출력 벡터를 곱하겠다. => 입력 시퀀스의 특정 부분에 관한 정보를 갖게 됨
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    encoder_optimizer = optim.SGD(encoder.paramters(), lr=learning_rate)
    ...
    for iter in range(1, n_iters + 1):
        ...
        # 입력 + 출력 쌍에서 입력을 input_tensor로 사용
        input_tensor = training_pairs[0]
        # 입력 + 출력 쌍에서 출력을 target_tensor로 사용
        target_tensor = training_pairs[1]
        loss = train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion)
        print_loss_total += loss
        plot_loss_total += loss
        
        # 모델을 75,000번 훈련을 진행하며 5,000번마다 오차를 출력
        if iter % 5000 == 0:
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for ei in range(input_length):
        encoder_output, encdoer_hidden = encoder(input_tensor[ei], encoder_hidden)
...
attn_model = trainIters(encoder1, attn_decoder1, n_iters=75000, print_every=5000, plot_every=100, learning_rate=0.01)